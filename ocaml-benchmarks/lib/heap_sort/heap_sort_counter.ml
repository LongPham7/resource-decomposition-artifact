...
(* Counter-related functions. We have two counters: (i) the first counter for
   tracking the recursion depth of the function heapify (which is renamed to
   heapify_build) inside the function repeatedly_heapify and (ii) the second
   counter for counting the recursion depth of the function heapify (which is
   renamed to heapify_extract) inside the function extract_list_from_heap. *)
...
let rec heapify_build (heap : int Rarray.t * Rnat.t) (index : int)
    two_current_original_counters =
...
let rec heapify_extract (heap : int Rarray.t * Rnat.t) (index : int)
    two_current_original_counters =
...
let rec extract_list_from_heap heap two_current_original_counters =
  let _ = Raml.tick 1.0 in
  let array, length = heap in
  Rnat.ifz length
    (fun () -> ([], two_current_original_counters))
    (fun length_minus_one ->
      ...
      in
      let _, counter1 = heapify_extract heap 0 initialized_counter in
      let counter2 = set_second_counter_to_zero counter1 in
      let recursive_result, counter2 =
        extract_list_from_heap (array, length_minus_one) counter2
      in
