...
let of_int (i : int) : t =
  if i < 0 then raise (Invalid_argument "Rnat.of_int : negative integer") else i

let add (a : t) (b : t) : t = a + b
let mult (a : t) (b : t) : t = a * b

let minus (a : t) (b : t) : t * t =
  if a <= b then raise (Invalid_argument "Rnat.minus : underflow")
  else (a - b, b)

let minusc (c : int) (n : t) : t * t =
  if n <= c then raise (Invalid_argument "Rnat.minusc : underflow")
  else (n - c, c)
