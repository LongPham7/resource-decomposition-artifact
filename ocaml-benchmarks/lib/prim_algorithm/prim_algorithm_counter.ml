...
let rec update_dist_all_neighbors heap list_neighbors
  ...
  let result, counter_final =
    ...
    | [] -> ((), new_counter)
    | (vertex, dist) :: tl ->
        let _, counter1 = decrease_key heap vertex (Some dist) new_counter in
...
let rec repeatedly_get_min_node adjacency_list heap acc
  ...
  if Rnat.to_int length = 0 then (acc, three_current_original_counters)
  else
    ...
    let _, counter2 =
      update_dist_all_neighbors heap list_neighbors initialized_counter
    in
...
let prim_algorithm adjacency_list three_current_original_counters =
  let _ = Raml.tick 1.0 in
