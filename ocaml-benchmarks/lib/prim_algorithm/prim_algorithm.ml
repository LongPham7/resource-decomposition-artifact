...
(* Distance type. The distance is either Infinity or Some d for some
   floating-point number d. At the beginning of Prim's algorithm, the shortest
   distances of all vertices are initialized to Infinity. *)
...
(* A pair of a vertex and a distance. This pair is stored inside each entry of a
   binary min-heap. In Prim's algorithm, the distance refers to the shortest
   distance between the vertex and some vertex already in the minimum spanning
   tree (MST) under construction. *)
...
(* I use an imperative (i.e., array-based) implementation of a binary min-heap,
   ...
   In Prim's algorithm, when we examine the neighbors of a vertex we just
   added to the MST, we would like to update the priority (i.e., distance) of
   those neighbors that are in the heap. This is where the decrease-key
   operation comes in handy.

   In a functional implementation of Prim's algorithm, we would need to create a
   new node and insert it to the heap, instead of updating an existing node in
   the heap. But in the imperative implementation, we can just update the
   priority of the node in the heap by the decrease-key operation. *)
...
(* Initialize the array inside the heap by placing vertex v (encoded by an
   ...
   the number of vertices in the graph. We also set the distances of all
...
(* Update the distances of all neighbors stored in the heap *)
...
let rec update_dist_all_neighbors heap list_neighbors =
  ...
  | [] -> ()
  | (vertex, dist) :: tl ->
      let _ = decrease_key heap vertex (Some dist) in
      update_dist_all_neighbors heap tl

(* Repeatedly extract the minimum node from the heap and put it in the MST *)
...
let rec repeatedly_get_min_node adjacency_list heap acc =
  ...
  if Rnat.to_int length = 0 then acc
    ...
    let _ = update_dist_all_neighbors heap list_neighbors in
