...
let analyze_prog analysis_mode m_name metric deg1 deg2 collect_fun_types e env =
  ...
  let rec analyze_exp deg deg_max =
    ...
    match
      try
        Analysis.analyze_expression e_normal_stack ~metric ~degree:deg
          ~collect_fun_types
      with Analysis.Anno_exn _ | Analysis.Analysis_error _ -> None
    with
    | None ->
...
let analyze_module analysis_mode m_name metric deg1 deg2 collect_fun_types
    ?(lp_vars_filename = None) fname m env =
  let amode_name =
    match analysis_mode with
    | Mupper -> "upper"
    | Mlower -> "lower"
  ...
  (* Parse a JSON file containing LP variables to filter out *)
  let list_lp_vars =
    match lp_vars_filename with
    | Some filename ->
        let lp_vars_json_string = Yojson.Basic.from_file filename in
        let open Yojson.Basic.Util in
        lp_vars_json_string |> to_list |> List.map ~f:to_int
    | None -> []
  in
  let analyze_fun f_name e =
    ...
    let rec analyze_f deg deg_max =
      ...
      match
        try
          Analysis.analyze_function e_normal_stack ~metric ~degree:deg
            ~collect_fun_types ~lp_int_vars_no_resource_guards:list_lp_vars
        with Analysis.Anno_exn _ | Analysis.Analysis_error _ -> None
      with
      | None ->
          let _ = if deg < deg_max then printf ", %!" else printf "\n%!" in
  ...
  let main_cont args action_p action_m =
    ...
    | [ "-m"; file; f_name ] | [ "-module"; file; f_name ] -> (
        if Sys.file_exists file <> `Yes then
          ...
          | None ->
              print_usage_error
                ("Top-level function '" ^ f_name ^ "' not found.")
  ...
  | action :: args -> (
      ...
      | "analyse" | "analyze" -> (
          ...
          | m_name :: deg_str :: args ->
              ...
              in
              let lp_vars_json_filename, args =
                match args with
                | "--lp-vars" :: filename :: args -> (Some filename, args)
                | _ -> (None, args)
              in
              let analyze_p =
              ...
              in
              let analyze_m =
                analyze_module analysis_mode m_name metric deg1 deg2 pmode
                  ~lp_vars_filename:lp_vars_json_filename
              in
