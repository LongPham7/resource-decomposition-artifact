...
module type SOLVER = sig
  ...
  val add_constr_list :
    ?lower:float -> ?upper:float -> (var * float) list -> unit

  val add_constr_array :
    ?lower:float -> ?upper:float -> (var * float) array -> unit
...
module Dummy_solver : SOLVER = struct
  exception E of string
...
module Clp_std_options : CLP_OPTIONS = struct
...
module Clp_std_maximize : CLP_OPTIONS = struct
  let row_buffer_size = 8000
...
module Clp (Options : CLP_OPTIONS) = struct
  ...
  let () = Clp.set_log_level !clp_state Options.log_level

  (* use n>0 for debugging *)
  ...
  let reset () = clp_state := Clp.create ()

  let get_num_vars () = Clp.number_columns !clp_state

  let get_num_constraints () = !num_rows + !row_count
  ...
  let fresh_var () =
    ...
    let () =
      if count % col_buffer_size = 0 then Clp.add_columns !clp_state many_cols
      else ()
    in
  ...
  let row_buffer =
    let init_row =
      { Clp.row_lower = 0.; Clp.row_upper = 0.; Clp.row_elements = [||] }
    in
  ...
  let flush_buffers () = flush_row_buffer ()

  let add_constr_array ?(lower = -.Float.max_value) ?(upper = Float.max_value)
      row_array =
    let () = if !row_count = row_buffer_size then flush_row_buffer () else () in
    ...
    in
    row_buffer.(!row_count - 1) <- row

  let add_constr_list ?(lower = -.Float.max_value) ?(upper = Float.max_value)
      row_list =
    let row_array = Array.of_list row_list in
    add_constr_array row_array ~lower ~upper

  let add_objective v q = objective := Map.set !objective v q

  let set_objective obj = objective := obj

  let reset_objective () = objective := Int.Map.empty

  let get_solution () = solution := Clp.primal_column_solution !clp_state

  let copy_objective () =
    let arr = Array.create (get_num_vars ()) 0.0 in
    let () = Int.Map.iteri !objective (fun ~key ~data -> arr.(key) <- data) in
  ...
  let first_solve () =
    ...
    get_solution ();
    match Clp.status !clp_state with 0 -> Feasible | _ -> Infeasible
  ...
  let get_objective_val () = Clp.objective_value !clp_state

  let get_solution v =
    if -1 < v && v < Array.length !solution then !solution.(v)
    else raise (E ("Variable " ^ string_of_int v ^ " is not in the solution."))
