...
module Make (Solver : SOLVER) (Amode : AMODE) = struct
  ...
  let rec forward : fun_anno_context -> analysis_expression -> forward_result =
   fun sigma exp ->
    ...
    | Elet (x_opt, e1, e2) -> (
        ...
        match x_opt with
        | None ->
            let _ = forward_closed sigma e1 in
  ...
  (* The optional parameter lp_int_vars_no_resource_guards is a list of LP
  variables whose corresponding indices do not contain resource guards *)
  let analyze_function ~degree ~metric ?(collect_fun_types = Pnone) exp
      ?(lp_int_vars_no_resource_guards = []) =
    let solve_with_obj solver_method qs =
      let module VMap = Anno.S.VMap in
      let objective =
        let f vmap q = VMap.set vmap q 1.0 in
    ...
    | Fclosed (Farrow fanno), Tarrow (targs, tres, _) -> (
        ...
        let qss =
          ...
          (* Print out indices and their corresponding LP variables *)
          (* let () =
            let print_index index =
              let v = tan_find atarg index in
              Format.printf "Index: %a var: %d\n" Pprint.fprint_index index
                (Anno.S.var_to_int v)
            in
            let print_list_indices is =
              List.iter is ~f:(fun x -> print_index x)
            in
            print_endline "Grouping of indices";
            List.iteri iss ~f:(fun group_id is ->
                printf "Group %d\n" group_id;
                print_list_indices is)
          in *)
          let f is = List.map ~f:(tan_find atarg) is in
          List.map ~f iss
        in
        match qss with
        | [] -> raise (Analysis_error "Empty index set.")
        | qs_max :: qss -> (
            (* I split the list qs_max of LP variables with the maximum
            polynomial degree into two parts: (i) LP variables whose indices
            containing no resource guards and (ii) and LP variables whose
            indices contain resource guards. In LP solving, I first minimize LP
            variables in the first group, followed by the second group. *)
            let qs_max, qss =
              let contains_no_resource_guards lp_var =
                List.exists lp_int_vars_no_resource_guards ~f:(fun x ->
                    Int.equal (Anno.S.var_to_int lp_var) x)
              in
              match lp_int_vars_no_resource_guards with
              | [] -> (qs_max, qss)
              | _ ->
                  let lp_vars_no_resource_guards =
                    List.map lp_int_vars_no_resource_guards ~f:(fun x ->
                        Anno.S.int_to_var x)
                  in
                  let qs_max_filtered_out =
                    List.filter qs_max ~f:(fun x ->
                        x |> contains_no_resource_guards |> not)
                  in
                  (lp_vars_no_resource_guards, qs_max_filtered_out :: qss)
            in
            try
